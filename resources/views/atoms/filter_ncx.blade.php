    ...
    <div class="flex flex-wrap items-end gap-4 mb-6">
        ...
        <div class="relative inline-block text-left flex-shrink-0">
            <h1 class="block text-sm font-medium text-gray-700 mb-1">Witel</h1>

            <button id="dropdown-witel-button"
                ...
                data-dropdown-id="dropdown-witel" data-name="witel">
                Pilih Opsi
            ...
            <div id="dropdown-witel"
                ...
                <div class="p-3">
                    ...
                    <div class="max-h-48 overflow-y-auto space-y-2 mb-3">
                        @foreach ($witelOptions as $witel)
                            <label class="flex items-center p-2 rounded hover:bg-gray-100 cursor-pointer">
                                <input type="checkbox" value="{{ $witel }}"
                                    class="option-checkbox w-4 h-4 text-blue-600 border-gray-300 rounded"
                                    {{ in_array($witel, $witelFilter ?? []) ? 'checked' : '' }}>
                                <span class="ml-2 text-sm text-gray-700">{{ $witel }}</span>
                            </label>
                        @endforeach
                    </div>
        ...
        <!-- Filter Product -->
        <div class="relative inline-block text-left flex-shrink-0">
            <h1 class="block text-sm font-medium text-gray-700 mb-1">Product</h1>

            <button id="dropdown-stream-button"
                class="filter-button flex items-center justify-between w-64 px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50"
                data-dropdown-id="dropdown-stream" data-name="product">
                Pilih Opsi
                <svg class="dropdown-icon w-5 h-5 ml-2 -mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                    fill="currentColor">
                    <path fill-rule="evenodd"
                        ...
                        @foreach ($productOptions as $product)
                            <label class="flex items-center p-2 rounded hover:bg-gray-100 cursor-pointer">
                                <input type="checkbox" value="{{ $product }}"
                                    class="option-checkbox w-4 h-4 text-blue-600 border-gray-300 rounded"
                                    {{ in_array($product, $productFilter ?? []) ? 'checked' : '' }}>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Pastikan periode input di-set
            if (selections.periode) {
                periodeInput.value = selections.periode;
            }
            ...
            buttons.forEach(button => {
                ...
                function updateButtonText() {
                    ...
                    if (count === 0) {
                        button.innerHTML =
                            `${defaultText}<svg class="dropdown-icon w-5 h-5 ml-2 -mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" /></svg>`;
                    } else if (count === 1) {
                        button.innerHTML =
                            `${selected[0]}<svg class="dropdown-icon w-5 h-5 ml-2 -mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" /></svg>`;
                    } else {
                        button.innerHTML =
                            `${count} selected<svg class="dropdown-icon w-5 h-5 ml-2 -mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" /></svg>`;
                    }
                }
                ...
                // Set status awal checkbox dari selections
                checkboxes.forEach(cb => {
                    if (selections[name].includes(cb.value)) {
                        cb.checked = true;
                    }
                });
            ...
            searchBtn.addEventListener('click', function() {
                ...
                if (selections.periode) {
                    params.append('periode', selections.periode);
                }

                if (selections.witel.length > 0) {
                    params.append('witel', selections.witel.join(','));
                }

                if (selections.product.length > 0) {
                    params.append('product', selections.product.join(','));
                }
            ...
            resetBtn.addEventListener('click', function() {
                ...
                buttons.forEach(button => {
                    ...
                    button.innerHTML =
                        `Pilih Opsi<svg class="dropdown-icon w-5 h-5 ml-2 -mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" /></svg>`;
                });
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @foreach (['region' => 'regionals', 'witel' => 'witels', 'product' => 'products'] as $key => $group)
                selections['{{ $key }}'] = [];
            @endforeach
