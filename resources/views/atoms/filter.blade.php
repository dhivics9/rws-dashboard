    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const selections = {
            periode: urlParams.get('periode') ?
                `${urlParams.get('periode').substring(0, 4)}-${urlParams.get('periode').substring(4)}` :
                "{{ $selectedPeriod }}"
        };
        ...
        buttons.forEach(button => {
            ...
            // Set nilai checkbox berdasarkan URL parameters
        ...
        resetBtn.addEventListener('click', function() {
            ...
            selections.periode = "{{ $selectedPeriod }}";
            periodeInput.value = selections.periode;
        ...
        // ====== Update Judul ======
        if (selections.periode && document.getElementById('report-title')) {
            const [year, month] = selections.periode.split('-');
            const monthNames = [
            ...
            document.getElementById('report-title').textContent =
                `Report Revenue – Product & Customer Summary (Periode: ${monthName} ${year})`;
        }
    });
</script>
