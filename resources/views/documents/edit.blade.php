...
@section('content')
            ...
            <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow">
                ...
                <div class="mb-5">
                    ...
                    <select name="document_type" id="document_type" onchange="showDynamicFields()"
                        ...
                        <option value="Berita Acara"
                            {{ old('document_type', $document->documentDetail->document_type) == 'Berita Acara' ? 'selected' : '' }}>
                            Berita Acara</option>
                        <option value="Resignation Letter"
                            {{ old('document_type', $document->documentDetail->document_type) == 'Resignation Letter' ? 'selected' : '' }}>
                            Resignation Letter</option>
                        <option value="Other Document"
                            {{ old('document_type', $document->documentDetail->document_type) == 'Other Document' ? 'selected' : '' }}>
                ...
                <div class="mb-5">
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500">{{ old('description', $document->documentDetail->description) }}</textarea>
                </div>
                ...
                <div id="resign-letter-fields"
                    class="hidden space-y-4 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <div>
                            ...
                            <textarea name="resign_letter[reason]" rows="3"
                                class="w-full border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500">{{ old('resign_letter.reason', $document->documentDetail->resignLetter->reason ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <!-- Custom File Name -->
                <div class="mb-5">
                    <label for="custom_file_name" class="block text-sm font-medium text-gray-700">Custom File Name
                        (Optional)</label>
                    <input type="text" name="custom_file_name" id="custom_file_name"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500"
                        placeholder="Masukkan nama custom tanpa ekstensi"
                        value="{{ old('custom_file_name', pathinfo($document->file_name, PATHINFO_FILENAME)) }}">
    ...
    <script>
        ...
        // Fungsi untuk update preview nama file
        function updateFileNamePreview() {
            const customName = document.getElementById('custom_file_name').value;
            const fileInput = document.getElementById('file');
            let extension = '{{ pathinfo($document->file_name, PATHINFO_EXTENSION) }}';
            let previewText = '';

            // Jika ada file baru dipilih, gunakan extension file baru
            ...
            if (customName) {
                previewText = customName + '.' + extension;
            } else if (fileInput.files.length > 0) {
                previewText = fileInput.files[0].name;
            } else {
                previewText = '{{ $document->file_name }}';
            }

            document.getElementById('file_name_preview').textContent = previewText;
        }

        // Event listeners
        document.getElementById('custom_file_name').addEventListener('input', updateFileNamePreview);
