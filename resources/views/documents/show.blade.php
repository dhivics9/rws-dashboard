...
<div class="p-6 bg-gray-50 min-h-screen">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex justify-between items-start mb-6">
            ...
            <a href="{{ route('documents.index') }}" class="px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-100 transition">← Back to List</a>
        </div>
        ...
        @if ($document->documentDetail->document_type === 'Berita Acara')
            <div class="bg-blue-50 p-5 rounded-lg border border-blue-200 mb-6">
                ...
                <dl class="grid grid-cols-1 gap-y-3">
                    <div><strong>Nama Pelanggan:</strong> {{ $document->documentDetail->beritaAcara->nama_pelanggan }}</div>
                    <div><strong>Lokasi Kerja:</strong> {{ $document->documentDetail->beritaAcara->lokasi_kerja }}</div>
                    <div><strong>Jenis Layanan:</strong> {{ $document->documentDetail->beritaAcara->jenis_layanan }}</div>
                    ...
                    <div><strong>Bandwidth Sebelumnya:</strong> {{ $document->documentDetail->beritaAcara->bw_prev }}</div>
                    <div><strong>Bandwidth Baru:</strong> {{ $document->documentDetail->beritaAcara->bw_new }}</div>
                    <div><strong>Tanggal Mulai:</strong> {{ $document->documentDetail->beritaAcara->tanggal_mulai }}</div>
                </dl>
            </div>
        @elseif ($document->documentDetail->document_type === 'Resignation Letter')
            <div class="bg-yellow-50 p-5 rounded-lg border border-yellow-200 mb-6">
                ...
                <dl class="grid grid-cols-1 gap-y-3">
                    <div><strong>Employee Name:</strong> {{ $document->documentDetail->resignLetter->employee_name }}</div>
                    <div><strong>Employee ID:</strong> {{ $document->documentDetail->resignLetter->employee_id }}</div>
                    <div><strong>Last Day of Work:</strong> {{ $document->documentDetail->resignLetter->last_day_of_work }}</div>
        ...
        @else
            <div class="bg-gray-50 p-5 rounded-lg border border-gray-200 mb-6">
                ...
                <p>This is a general document with no specific details. Please refer to the description and the file preview.</p>
            </div>
        @endif
